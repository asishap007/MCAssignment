...
namespace GameReserveApp.Repository
{
    class CategoryRepository
    {
        ...
        public static CategoryView AddCategory(CategoryView category)
        {
            string caetgoryurl = config.serverUrl + config.addCategoryUri;
            string postData = JsonConvert.SerializeObject(category);
            string addCategoryInJson = RequestClient.postRequest(caetgoryurl, postData);
            CategoryView addCategory = JsonConvert.DeserializeObject<CategoryView>(addCategoryInJson);
            return addCategory;
        }
